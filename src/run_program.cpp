...
error_t run_program_argp_parse_opt (int key, char *arg, struct argp_state *state)
{
	...
	{
		case 'T':
			config->time_limit = round(stod(arg) * 1000) / 1000;
			break;
		case 'R':
			config->real_time_limit = round(stod(arg) * 1000) / 1000;
			break;
		case 'M':
...
void run_child() {
	set_limit(RLIMIT_CPU, ceil(run_program_config.time_limit), ceil(run_program_config.real_time_limit));
...
RunResult trace_children() {
	...
	if (rp_timer_pid == -1) {
		...
		struct timespec ts;
		ts.tv_sec = ceil(run_program_config.real_time_limit);
		ts.tv_nsec = 0;
		nanosleep(&ts, NULL);
		exit(0);
	}
